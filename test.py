...
def show_ens(predict_save,input_img, labs, save_path):
    ...
    cv2.drawContours(input_img, contours=lbl_contour, contourIdx=-1, color=(0, 0, 255),thickness=2)  # red
    pred_contour, hierarchy = cv2.findContours(predict_save.astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    cv2.drawContours(input_img, contours=pred_contour, contourIdx=-1, color=(0, 255, 0),thickness=2)  # green
...
def test_each_fold(val_filelists, model, test_num):
    ...
    with tqdm(total=test_num, desc='Test visualize', unit='img', ncols=70, leave=True) as pbar:
        for i, (sampled_batch, names) in enumerate(test_loader, 1):
            ...
            lab_img = cv2.imread(config.train_dataset+"labelcol/"+names[0][:-3]+"png",0)
...
if __name__ == '__main__':
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(filelists)):
        ...
        model_path = "./NIH_Pancreas_kfold/"+model_type+"/"+test_session+"/models/fold_"+str(fold+1)+"/best_model-"+model_type+".pth.tar"
        ...
        if model_type == 'MMDUNet':
            model = MMDUNet(n_channels=config.n_channels, n_classes=config.n_labels)
